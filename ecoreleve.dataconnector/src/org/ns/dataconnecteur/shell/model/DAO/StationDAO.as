package org.ns.dataconnecteur.shell.model.DAO
{
	...
	*/
	...
	{		
		private static const sqlCreate:String						='CREATE TABLE IF NOT EXISTS Station ([sta_id] integer PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE, [sta_name] text, [sta_latitude] float NOT NULL, [sta_longitude] float NOT NULL, [sta_elevation] float, [sta_date] date NOT NULL, [sta_nbIndividual] integer, [sta_speciesName] text, [sta_comments] text, [sta_dateY] TEXT, [sta_dateYM] TEXT, [sta_dateYMD] TEXT, [sta_fkQuery] INT, [sta_source] VARCHAR);';
		...
		private static const sqlDeleteRow:String					='DELETE FROM Station WHERE Station.sta_id = @sta_id;'
		...
		//INSERT--------------------------------------------------------------------------------------------
		public function insertRow( rowItem:StationVO, resultHandler:Function = null, faultHandler:Function = null ):void 
		{
			var stmt:SQLStatement = new SQLStatement();
			stmt.sqlConnection = sqlConnection;
			stmt.text = sqlInsert
			...
			stmt.addEventListener( SQLEvent.RESULT,
			function ( event:SQLEvent ):void {
				if (!rowItem.sta_id > 0) rowItem.sta_id = stmt.getResult().lastInsertRowID;
		...
		//DELETE ------------------------------------------------------------------------
		public function deleteRow( rowItem:StationVO, resultHandler:Function = null, faultHandler:Function = null ):void 
		{
			var stmt:SQLStatement = new SQLStatement();
			stmt.sqlConnection = sqlConnection;
			stmt.text = sqlDeleteRow;
			var params:Array = [ {name:"sta_id", value:rowItem.sta_id} ];
			setParameters( stmt, params );
			stmt.addEventListener( SQLEvent.RESULT,
			function ( event:SQLEvent ):void {
				if (resultHandler != null) resultHandler.call(this, rowItem);
