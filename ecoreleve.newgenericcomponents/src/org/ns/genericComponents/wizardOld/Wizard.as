package org.ns.genericComponents.wizardOld
{
	...
	import org.ns.genericComponents.wizardOld.events.WizardEvent;
	import org.ns.genericComponents.wizardOld.interfaces.IWizardStep;
	import org.ns.genericComponents.wizardOld.skin.WizardFilArianeContainerSkin;
	import org.ns.genericComponents.wizardOld.skin.WizardMainContainerSkin;
	import org.ns.genericComponents.wizardOld.skin.WizardTitleWindowSkin;
	...
	public class Wizard extends TitleWindow
	{
		...
		private var _containerMain:SkinnableContainer;
		private var _containerFilArianne:SkinnableContainer;
		...
		private var WizardMainContainerSkinClass:Class=WizardMainContainerSkin;
		private var WizardFileArianeSkinClass:Class=WizardFilArianeContainerSkin;
		...
		override protected function createChildren():void 
		{
			...
			//cretae group for fil d'arianne
			_containerFilArianne=new SkinnableContainer();
			_containerFilArianne.percentHeight=100;			
			
			//create group for the main content of wizard
			_containerMain=new SkinnableContainer();
			...
			if (_stepsViewStack == null) {
				...
				_stepsViewStack.addEventListener(FlexEvent.CREATION_COMPLETE, onStepsViewStackCreationComplete, false, 0, true);
				_containerMain.addElement(_stepsViewStack);
			}
			
			//add the two group
			this.addElement(_containerFilArianne)
			this.addElement(_containerMain)
		...
		{
			//if WizardTitleWindow skin is default ==> add component customskin
			var c:Class=getStyle("skinClass");
			
			if (c==spark.skins.spark.TitleWindowSkin){
				this.setStyle("skinClass",WizardSkinClass)
			}
			
			//if "fil d'ariane" skin is default ==> add component customskin
			var d:Class=_containerFilArianne.getStyle("skinClass");
			
			if (d==spark.skins.spark.SkinnableContainerSkin){
				_containerFilArianne.setStyle("skinClass",WizardFileArianeSkinClass)
			}

			//if mainContainer skin is default ==> add component customskin
			var e:Class=_containerMain.getStyle("skinClass");
			
			if (e==spark.skins.spark.SkinnableContainerSkin){
				_containerMain.setStyle("skinClass",WizardMainContainerSkinClass)
			}
		...
		private function initWizardSteps():void 
		{
			...
			for each (var step:IWizardStep in steps) {
				...
				if (step.stepName!=null){
					...
					_containerFilArianne.addElement(lbl);
					i++;
				}			
			}		
		...
		{
			var count:uint = _containerFilArianne.numElements;			
			for (var i:int = 0; i < count; i++){		
				if (_containerFilArianne.getElementAt(i) is Label){
					var lbl:Label=_containerFilArianne.getElementAt(i) as Label;
		...
		private function scaleStepsViewStack():void 
		{
			...
			for (var j:int = 0; j < count; j++){		
				if (_containerFilArianne.getElementAt(j) is Label){
					var lbl:Label=_containerFilArianne.getElementAt(j) as Label;
			...
			_containerFilArianne.width=maxFilArianeWidth + 15;
			_containerFilArianne.invalidateSize();
