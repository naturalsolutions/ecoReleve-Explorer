package HierachicalADG
{
	...
	import HierachicalADG.event.SelectEvent;
	...
	public class HierachicalADG extends VBox
	{
		...
		[Inspectable(category="General", type="int", defaultValue="2")]
		private var _nbColToSee:int=2;
		...
		[Bindable]
		public function get nbColToSee():int
		{
			return _nbColToSee;
		}
		
		public function set nbColToSee(value:int) : void 
		{
			_nbColToSee=value
		}
		...
		override protected function commitProperties():void
		{
		 	...
		 	if(ADGSourceChanged == true){		  		
		  	...
		  	if(colToUseChanged == true){
				if (_colToUse.length==0){
					columnEditor.enabled=false
				}else{
					columnEditor.enabled=true
					MajADGDataSource()
				}
		...
		**/
		override protected function createChildren():void 
		{
			...
			adg.addEventListener(ListEvent.ITEM_CLICK,clickRowHandler);
		...
		private function clickRowHandler(event:ListEvent):void
		{
			adg.dispatchEvent(new SelectEvent('select',true,false,adg.selectedItems));
		}
